...
async function fetchFromAppsScript(path, method = 'GET', body = null, queryParams = {}) {
    ...
    // Para requisições GET, anexamos o 'path' diretamente à URL base.
...
app.get('/get-membros', async (req, res) => {
    try {
        ...
        const data = await fetchFromAppsScript('get-membros');
...
app.get('/get-presencas-total', async (req, res) => {
    try {
        ...
        const data = await fetchFromAppsScript('get-presencas-total', 'GET', null, req.query);
...
app.get('/get-all-last-presences', async (req, res) => {
    try {
        ...
        const data = await fetchFromAppsScript('get-all-last-presences'); // Chama a nova função do Apps Script
        res.status(200).json(data.data || {}); // Apps Script retorna { success: true, data: {...} }
    } catch (error) {
...
app.post("/login", async (req, res) => {
    ...
    try {
        // Alinhado com o 'case 'get-membros':' no Apps Script doGet
        const responseData = await fetchFromAppsScript('get-membros');
